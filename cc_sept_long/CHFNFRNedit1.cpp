...
//ifstream infile("graphedit1.txt");
//ofstream outfile("cliquesedit1.txt");
...
int main()
{
    ...
    while(testcase--){
        ...
        stringstream ss(*tempit); // Insert the string into a stream
        ...
        for(vecit = tokens.begin(); vecit != tokens.end(); vecit++){
            string tempstr = *vecit;
            stringstream convert(tempstr);//object from the class stringstream.
            convert>>maxnodes[counter1++]; 
        }
        ...
        string findstr = ssnew.str();
        

        if(st.find(findstr) != st.end() || findstr.size() == 2)
            cout<<"Yes"<<endl;
        else
            cout<<"No"<<endl;
